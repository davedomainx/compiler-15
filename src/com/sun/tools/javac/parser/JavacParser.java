...
/* ---------- parsing -------------- */
    ...
    private boolean allowedAfterSealedOrNonSealed(Token next, boolean local, boolean currentIsNonSealed) {
        // WARATEK MODIFIED: commented the following out for Java 9 compatibility
/*
        return local ?
            switch (next.kind) {
...
*/
        // WARATEK MODIFIED: replaced the above for Java 9 compatibility
        if (local) {
            switch (next.kind) {
                case MONKEYS_AT:
                case ABSTRACT:
            ...
            switch (next.kind) {
                ...
                case ENUM:
                    return true;
                case IDENTIFIER:
                    return isNonSealedIdentifier(next, currentIsNonSealed ? 3 : 1) || next.name() == names.sealed;
                default:
                    return false;
            }
        }
